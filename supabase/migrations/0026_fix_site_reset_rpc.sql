-- Migration: 0026_fix_site_reset_rpc.sql
-- Description: Fixes "DELETE requires a WHERE clause" error by adding WHERE TRUE
--              to all unconditional DELETE statements inside the reset function.
...
BEGIN
  ...
  -- ── 5. Clear waitlist ──────────────────────────────────────────────────────
  DELETE FROM public.waitlist WHERE TRUE;
  GET DIAGNOSTICS v_count = ROW_COUNT;
  ...
  DELETE FROM public.communication_events WHERE TRUE;
  GET DIAGNOSTICS v_count = ROW_COUNT;
  ...
  DELETE FROM public.verification_reminders WHERE TRUE;
  GET DIAGNOSTICS v_count = ROW_COUNT;
  v_deleted := v_deleted || jsonb_build_object('verification_reminders_cleared', v_count);

  -- ── 11. Clear draft/archived/scheduled campaigns only ─────────────────────
  ...
  -- ── 13. Clear fraud logs ──────────────────────────────────────────────────
  DELETE FROM public.fraud_logs WHERE TRUE;
  GET DIAGNOSTICS v_count = ROW_COUNT;
  ...
  -- ── 15. Clear audit_logs (fresh slate), then log the reset itself ─────────
