...
serve(async req => {
  ...
  try {
    ...
    if (!Deno.env.get('SUPABASE_ANON_KEY'))
      throw new Error('Server Config Error: SUPABASE_ANON_KEY missing')
    if (!supabaseServiceKey)
      throw new Error('Server Config Error: SUPABASE_SERVICE_ROLE_KEY missing')
    ...
    if (profile?.role !== 'super_admin') {
      throw new Error(
        `Forbidden: Only super admins can invite other admins (Your role: ${profile?.role})`
      )
    }
    ...
    const { data: existingUserId, error: existError } = await adminClient.rpc(
      ...
      const { data: inviteData, error: inviteError } =
        await adminClient.auth.admin.inviteUserByEmail(email, {
          ...
          data: {
            ...
            invited_by: user.id,
            invited_by_name: profile?.full_name || 'An Admin',
          },
        })
    ...
    // 6. Emit domain event for Communication Infrastructure (only for promotions, invites are handled by Auth Hook)
    if (existingUserId) {
